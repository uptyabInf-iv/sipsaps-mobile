...
export default memo(function TarjetaCita({ item, onPress, onViewDetails }) {
  ...
  const fmtFecha = (d) =>
    d && !isNaN(d) ? d.toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
      ...
      <View style={{ flex: 1, padding: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <View style={{ flex: 1, paddingRight: 12 }}>
            <Text style={{ fontSize: 18, fontWeight: '700', color: colores.textoPrincipal }} numberOfLines={1}>
              {item?.paciente?.nombre ?? '—'}
            </Text>
            {item?.paciente?.cedula ? (
              <Text style={{ marginTop: 2, color: colores.textoSecundario, fontSize: 12 }} numberOfLines={1}>
                {item.paciente.cedula}
              </Text>
            ) : null}
          </View>

          <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 6, paddingHorizontal: 10, borderRadius: 10, borderWidth: 1, borderColor: infoEstatus.color, backgroundColor: chipBg(infoEstatus.color) }}>
            <FontAwesome name={infoEstatus.icon} size={14} color={infoEstatus.color} />
            <Text style={{ marginLeft: 6, fontWeight: '700', color: colores.textoPrincipal }}>{status}</Text>
          </View>
        </View>

        <View style={{ flexDirection: 'row', marginTop: 10 }}>
          <View style={{ backgroundColor: colores.principal + '10', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: colores.principal }}>
            <Text style={{ color: colores.principal, fontWeight: '700', fontSize: 12 }} numberOfLines={1}>
        ...
        {item?.justificacion ? (
          <View style={{ marginTop: 12, borderWidth: 1, borderColor: '#ECECEC', borderRadius: 10, padding: 10, backgroundColor: esOscuro ? '#111' : '#FAFAFA' }}>
            <Text style={{ color: colores.textoSecundario, fontSize: 12, marginBottom: 4 }}>Justificación</Text>
        ...
        <View style={{ gap: 8 }}>
          {fechaProgramada ? (
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: chipBg('#17a2b8'), borderWidth: 1, borderColor: '#17a2b8' }}>
          ...
          {fueReprogramada && fechaProgramada ? (
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: chipBg('#ff9800'), borderWidth: 1, borderColor: '#ff9800' }}>
          ...
          {fechaAtendida ? (
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: chipBg('#28a745'), borderWidth: 1, borderColor: '#28a745' }}>
                <FontAwesome name="stethoscope" size={14} color="#28a745" />
              </View>
              <View style={{ marginLeft: 10 }}>
                <Text style={{ color: colores.textoSecundario, fontSize: 12 }}>Atendida</Text>
                <Text style={{ color: colores.textoPrincipal, fontWeight: '700' }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 14 }}>
          ...
          <TouchableOpacity
            ...
            style={{ backgroundColor: colores.principal, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, flexDirection: 'row', alignItems: 'center' }}
          >
